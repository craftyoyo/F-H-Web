@extends('layouts.app')

@section('title', 'A propos')

@section('content')
<!-- SUBHEADER -->
<div id="subheader" class="shared">
  <div class="subheader-text">
    <h1>Infrastructure VPN Haute Performance</h1>
    <h2>Combiné avec notre support 24x7x365</h2>
  </div>
</div>
<!-- END OF SUBHEADER -->

<!-- PRICE BOXES -->
<div class="pricingbox shared">

 <div class="row">
   <div class="col-sm-12">
     <span class="chooseprice">Prix par préiode</span>
     <div class="price-per-period">
      <button type="button" class="btn btn-shared-checked ripple permonth">Mois</button>
      <button type="button" class="btn btn-default ripple peryear">6 Mois</button>
      <button type="button" class="btn btn-default ripple per2yrs">12 Mois</button>
    </div>
  </div>
</div>


<div class="row spacing-75">

  <div class="col-sm-4">
    <div class="pricing-plan red">
      <div class="pricing-title">Gratuit</div>
      <div class="pricing-price">
        <div class="monthprice">0.00€<span>/m</span></div>
        <div class="yearprice">0.00€<span>/6M</span></div>
        <div class="twoyearprice">0.00€<span>/12M</span></div>
      </div>
      <div class="pricing-features">
        <ul>
          <li>1Mb/s</li>
          <li>10GB Transfert par mois</li>
	        <li>1 devices MAX</li>
          <li>10H-20H Lu-Je Support Ticket 6H MAX</li>
        </ul>
      </div>
      <div class="text-center"><a class="mtr-btn button-raised ripple" href="javascript:void(0)">Acheter!</a></div>
    </div>
  </div>

  <div class="col-sm-4">
    <div class="pricing-plan red">
      <div class="pricing-title">Start</div>
      <div class="pricing-price">
        <div class="monthprice">3.99€<span>/m</span></div>
        <div class="yearprice">19.99€<span>/6M</span></div>
        <div class="twoyearprice">39.99€<span>/12M</span></div>
      </div>
      <div class="pricing-features">
        <ul>
         <li>5Mb/s</li>
         <li>500Gb</li>
         <li>3 devices MAX</li>
         <li>10H-20H Lu-Je Support TicketTicket 4H MAX</li>
        </ul>
      </div>
      <div class="text-center"><a class="mtr-btn button-raised ripple" href="">Bientôt</a></div>
    </div>
  </div>

  <div class="col-sm-4">
    <div class="pricing-plan red">
      <div class="pricing-title">Pro</div>
      <div class="pricing-price">
        <div class="monthprice">5.99€<span>/m</span></div>
        <div class="yearprice">29.99€<span>/6M</span></div>
        <div class="twoyearprice">59.99€<span>/12M</span></div>
      </div>
      <div class="pricing-features">
        <ul>
          <li>illimité</li>
          <li>Transfert ILIMITE</li>
          <li>10 devices MAX</li>
          <li>24/7/365 Support tel,sms,ticket 1H MAX</li>
        </ul>
      </div>
      <div class="text-center"><a class="mtr-btn button-raised ripple" href="">Bientôt</a></div>
    </div>
  </div>

<div class="spacing-75"></div>
  </div>
</div>

</div>
<!-- END OF PRICE BOXES -->

<!-- COMPARISON TABLE -->

<div class="shared-table">

 <p class="text-center">Comparer nos Offres</p>

 <div class="row spacing-25">
  <div class="col-sm-12">
    <table class="products-table responsive tablesaw tablesaw-stack" data-tablesaw-mode="stack" style="display: table;">
      <thead>
        <tr>
          <th></th>
          <th>Gratuit</th>
          <th>Start</th>
          <th>Pro</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>débit</td>
          <td>1mb/s</td>
          <td>5Mb/s</td>
          <td>illimité (1Gb/s limite du serveur)</td>
        </tr>

        <tr>
          <td>Nombre de client</td>
          <td>1</td>
          <td>3</td>
          <td>10</td>
        </tr>

        <tr>
          <td>Traffic sécurisé</td>
          <td><i class="fa fa-check"></i></td>
          <td><i class="fa fa-check"></i></td>
          <td><i class="fa fa-check"></i></td>
        </tr>

        <tr>
          <td>Support</td>
          <td>10H-20H Lu-Je Support Ticket 6H SLA</td>
          <td>10H-20H Lu-Je Support Ticket 3H SLA</td>
          <td>24H-24H Lu-Di Support Ticket/tel/sms 1H SLA</td>
        </tr>

        <tr>
          <td>Bande passente</td>
          <td>10GB</td>
          <td>500GB</td>
          <td>Ilimité</td>
        </tr>

		<tr>
          <td>24x7 Support</td>
          <td><i class="fa fa-times"></i></td>
          <td><i class="fa fa-times"></i></td>
          <td><i class="fa fa-check"></i></td>
        </tr>

        <tr class="price-comparison">
          <td>Prix par mois</td>
          <td>0.00€</td>
          <td>3.99€</td>
          <td>5.99€</td>
        </tr>

        <tr class="price-comparison">
          <td>Prix pour 6 mois</td>
          <td>0.00€</td>
          <td>19.99€</td>
          <td>29.99€</td>
        </tr>

        <tr class="price-comparison">
          <td>Prix pour 12 mois</td>
          <td>0.00€</td>
          <td>39.99€</td>
          <td>59.99€</td>
        </tr>

        <tr>
          <td></td>
          <td><a class="mtr-btn button-fab ripple btn-sm" href="">ORDER NOW</a></td>
          <td><a class="mtr-btn button-fab ripple btn-sm" href="">Bientôt</a></td>
          <td><a class="mtr-btn button-fab ripple btn-sm" href="">Bientôt</a></td>
        </tr>

      </tbody>
    </table>
  </div>
</div>

</div>
<!-- END OF COMPARISON TABLE -->

<!-- VIDEO BACKGROUND -->
<div id="shared-video" class="hidden-xs">
  <div class="shared-video-container">
    <div class="videocaption">
     <div class="videocaption-content  wow fadeIn" data-wow-duration="500ms" data-wow-delay="200ms">
      <h2>By Developers, for Developers</h2>
      <hr/>
      <p></p>
    </div>

  </div>
  <video autoplay loop muted poster="images/video-poster.jpg"  class="hidden-xs">

      <source src="video/fh-vid.webm" type="video/webm">

        </video>
      </div>
    </div>
    <!-- END OF VIDEO BACKGROUND -->

@endsection